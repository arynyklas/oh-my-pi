...
async function commandsForMode(mode: Mode): Promise<TestPlan> {
	switch (mode) {
		case "workspace":
			return testPlan(fastWorkspacePackages.map(pkg => workspaceTestCommand(pkg, 8)));
		case "native":
			return testPlan(nativeAndIntegrationPackages.map(pkg => workspaceTestCommand(pkg, 4)));
...
if (import.meta.main) {
	...
	const requestedCommands = selectShard(plan.commands, Bun.env.OMP_TEST_SHARD);
