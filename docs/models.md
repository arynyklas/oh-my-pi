...
`openai-images`, `openai-speech`, and `openai-transcriptions` are runner APIs, not chat transports. A model declared with one is catalogued as the matching kind and selected by that model role instead of the chat picker:

| API | Endpoint | Kind | Role |
| --- | --- | --- | --- |
| `openai-images` | `{baseUrl}/images/generations` (`/images/edits` with input images) | `image` | `image` |
| `openai-speech` | `{baseUrl}/audio/speech` | `tts` | `speech` |
| `openai-transcriptions` | `{baseUrl}/audio/transcriptions` (multipart) | `stt` | `dictation` |

Chat token defaults (`contextWindow`, `maxTokens`) are left unset for these models. Under `transport: pi-native` they target the gateway's `/v1` root.
...
- `transport`: `pi-native` only. When set, every chat model under that provider is sent to an `omp auth-gateway` compatible `baseUrl` via `POST /v1/pi/stream`; image, speech, and transcription runners go to the same gateway's `/v1/<task>` routes instead. `apiKey` is the gateway bearer.
...
Rows an omp auth-gateway marks with a non-chat `kind` skip that routing and use the gateway's task route instead: `image` → `openai-images` (`image` role), `tts` → `openai-speech` (`speech` role), `stt` → `openai-transcriptions` (`dictation` role). Other runner kinds are not offered as chat models.
