...
export async function initDb(): Promise<Database> {
	...
	// Parse cursor for `offset` (file identity, tail hash, and the service tier
	// active there), so an incremental sync of a huge transcript neither
	// re-reads the prefix to recover the tier nor trusts an offset into a file
	// that was rewritten. Rows written before this column exists stay SQL NULL,
	// which `getFileOffset` reports as "no cursor" — one full re-parse.
 ...
 *
 * `parserState` is absent for a row that predates cursor persistence or whose
 * JSON no longer matches `offset`; the parser then re-reads the file from byte
 * 0 instead of trusting the offset (and replays the prefix to recover the
 * service tier active there).
 */
...
export function applySessionParseResult(
	...
	return database.transaction(() => {
		...
		// A chunk that did not reach end of file must not record the file's own
		// mtime: the skip guard in `syncAllSessionsLocked` would then read the
		// partially ingested transcript as fully synced. Storing 0 keeps the
		// committed offset and its cursor while forcing the next sync to resume.
		setFileOffset(sessionFile, result.newOffset, result.done ? parserState.mtimeMs : 0, parserState);
