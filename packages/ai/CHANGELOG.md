...
- Auth gateway `GET /v1/models` rows now advertise `reasoning` and `thinking_efforts` (the served model's effort ladder) so clients can offer the levels the gateway actually honors.

### Fixed

- Fixed Claude Opus 5.5 failing the first tool-forcing request with `400 tool_choice: type "tool" and "any" are not supported for this model`; forced tool choice is now downgraded to `auto` for Opus 5.5 on every host, which also keeps thinking at the requested effort.
...
## [18.2.4-fork.1] - 2026-09-17

### Changed

- Re-based the fork's credential selection policy and per-provider default-account handling onto upstream v18.2.4's session-state store and generalized usage-block healing; `AuthStorage.login` keeps the fork's shared `acquireAuthCredential` helper and now forwards upstream's `onBrowserSession` controller, so Perplexity login works from every login surface.
...
- Synced the fork onto upstream v18.1.16: the fork's per-provider default-account and cross-type credential rotation absorb upstream's Codex SSE replay-safe retries, GitHub Copilot Enterprise OAuth app selection and plan/model-policy 403 credential preservation, reasoning-effort fallback phrasing fixes, and Muse Code subscription sign-in.
...
- Claude accounts now recover from stale quota cooldowns when fresh usage confirms available capacity, without clearing fresh rate limits or still-exhausted model tiers.

## [18.1.11-fork.1] - 2026-09-06

### Changed

- Synced the fork onto upstream v18.1.11: the fork's cross-type credential rotation now carries upstream's provider-timed block provenance and merged `blockedUntilMs`/`reportResetAtMs` deadlines, and the fork's shared OAuth login helper honors upstream's abort-aware paste-code prompt.
...
- Fixed a session's pinned OAuth account being dropped on restart: the fork's positional-assignment reset ran on a provider's first credential load, which also cleared the persisted session-sticky rows a restarted process reads back. The reset now runs only on a real reorder.
