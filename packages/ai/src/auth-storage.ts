...
type ApiKeySelection = CredentialSelection<ApiKeyCredential> & { id: number };
...
export class AuthStorage {
	...
	#getSessionCredential(
		...
		if (sessionMap?.has(sessionId)) {
			...
			if (
				...
				return {
					type: sessionCredential.type,
					index: actualIndex,
					credentialId: actual.id,
					lastUsedAtMs: sessionCredential.lastUsedAtMs,
				};
			}
	...
	async #selectApiKeyCredential(
		...
		const candidate = candidates[0]?.selection;
		if (!candidate) return fallback;
		const id = this.#getStoredCredentials(provider)[candidate.index]?.id;
		return id === undefined ? fallback : { ...candidate, id };
	}
	...
	async login(
		...
		await this.#upsertOAuthCredential(acquired.provider, newCredential);
		return {
			type: "oauth",
			email: acquired.credential.email,
			accountId: acquired.credential.accountId,
			orgId: acquired.credential.orgId,
			orgName: acquired.credential.orgName,
		};
	}
	...
	async #resolveCredentialTarget(
		provider: string,
		sessionId: string | undefined,
		options?: { credentialId?: number; apiKey?: string; selection?: AuthCredentialSelectionPolicy },
		...
		if (options?.credentialId !== undefined) {
			...
			if (entry && this.#isCredentialIdEligible(entry.id, options.selection)) {
		...
		if (options?.apiKey !== undefined) {
			...
			for (let index = 0; index < stored.length; index++) {
				...
				if (
					entry &&
					this.#isCredentialIdEligible(entry.id, options.selection) &&
		...
		const sessionCredential = this.#getSessionCredential(provider, sessionId, options?.selection);
	...
	async markUsageLimitReached(
		...
		if (!sessionCredential && options?.credentialId === undefined && options?.apiKey !== undefined) {
			...
			const index =
				credentialId === undefined || !this.#isCredentialIdEligible(credentialId, selection)
					? -1
	...
	async #resolveOAuthSelectionMode(
		...
		let rankingOrder =
			shouldRank && sessionId && selectionPolicy?.strategy !== "least-used" ? baseRankingOrder : order;
		...
		if (shouldKeepOAuthSticky && !shouldRank && sessionPreferredIndex !== undefined && !hasPlanRequirement) {
	...
	async resolveApiKeySelection(
		...
		if (!selection) {
			...
			const configKey = this.#configOverrides.get(provider);
			if (configKey !== undefined) {
				const resolvedConfigKey = await this.#configValueResolver(configKey);
	...
	inheritSessionCredentials(sourceSessionId: string, targetSessionId: string): number {
		...
		for (const provider of this.#data.keys()) {
			...
			this.#recordSessionCredential(provider, targetSessionId, credential.type, credential.index, {
