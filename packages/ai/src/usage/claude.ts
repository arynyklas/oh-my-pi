...
export const claudeRankingStrategy: CredentialRankingStrategy = {
	 ...
	 * and the block then idles a usable account for days. A report may only lift
	 * a block when it proves recovery: partial evidence, malformed counters, or a
	 * still-spent shared wall keep it.
	 */
	healableBlockScopes(report) {
		...
		// Only Fable/Mythos can hold a scoped block ({@link blockScope} scopes
		// reactive blocks for those tiers alone), so any other tier scope would
		// name a block that cannot exist.
		const tiers = new Set(
			report.limits.map(limit => limit.scope.tier).filter(tier => tier === "fable" || tier === "mythos"),
		);
